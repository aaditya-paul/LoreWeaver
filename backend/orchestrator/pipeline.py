...
class StoryOrchestrator:
    ...
    def generate_next_scene(
        ...
        # -- Phase 2: Planning (via selected LLM) -----------------------------
        llm_name = type(self.llm).__name__
        log.info(f"[Phase 2] Sending to {llm_name} Planner...")
        t = time.perf_counter()
        outline = self.llm.plan_scene(tier_1_state, tier_2_memory, user_prompt)
        log.info(f"   {llm_name} Planner done in {time.perf_counter()-t:.2f}s")
        if "error" in outline:
            log.error(f"   Planner FAILED: {outline}")
        ...
        for attempt in range(max_retries):
            log.info(f"[Phase 4] Execution attempt {attempt + 1}/{max_retries}  ->  {llm_name}...")
            t = time.perf_counter()
            draft_scene = self.llm.generate_scene(full_context, outline)
            elapsed = time.perf_counter() - t
            log.info(f"   {llm_name} done in {elapsed:.2f}s  ({len(draft_scene)} chars generated)")
            log.debug(f"   Draft (first 500 chars):\n{draft_scene[:500]}")

            log.info(f"[Phase 4] Running {llm_name} Consistency Critic...")
            t = time.perf_counter()
            critic_report = self.llm.evaluate_consistency(tier_1_state, draft_scene)
